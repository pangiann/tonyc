...
expr        : atom                                      { { expr_info = E_atom ($1);     expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false }  }
            | T_const                                   { { expr_info = E_const($1);     expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false }  }
          ...
          /*  | unary_op expr %prec T_neg                 { { expr_info = E_unary_op ($1, $2); expr_error_pos = ($startpos, $endpos) }  }*/
            ...
            | T_new mytype T_lbracket expr T_rbracket   { { expr_info = E_new ($2,  $4);   expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false } }
            ...
            | T_nil                                     { { expr_info = E_nil;             expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false } }

            | T_nil_qm T_lparen expr T_rparen           { { expr_info = E_nilqm($3);       expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false } }
            | expr T_hashtag expr                       { { expr_info = E_hashtag($1, $3); expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false } }
            | T_head T_lparen expr T_rparen             { { expr_info = E_head($3);        expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false } }
            | T_tail T_lparen expr T_rparen             { { expr_info = E_tail($3);        expr_error_pos = ($startpos, $endpos); expr_type = TYPE_none; expr_callbyref = false } }
