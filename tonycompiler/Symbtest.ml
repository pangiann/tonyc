...
let rec pretty_typ ppf typ =
  match typ with
  | TYPE_none -> fprintf ppf "undefined "
  | TYPE_int ->
    fprintf ppf "int "
  | TYPE_char ->
    fprintf ppf "char "
...
let main =
   ...
   let s1 = lookupEntry (id_make "s1") LOOKUP_ALL_SCOPES true in ignore s1;
   let i2 = lookupEntry (id_make "i2") LOOKUP_ALL_SCOPES true in ignore i2;
   let i1 = lookupEntry (id_make "i1") LOOKUP_ALL_SCOPES true in

   fprintf ppf "%d" i1.entry_info.variable_offset;
