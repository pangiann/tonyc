...
let newEntry id inf func_entry_info err =
  try
    if err then begin
      try
    ...
    begin
      match func_entry_info with
      | ENTRY_function function_info ->
        function_info.var_type_list <-
          ( match inf with
            | ENTRY_variable  var_info -> inf :: function_info.var_type_list;
            | ENTRY_parameter par_info -> inf :: function_info.var_type_list;
            | _ -> function_info.var_type_list
          );
        ()
      | _ -> ()
    end;
...
let newVariable id typ func_entry err =
  !currentScope.sco_negofs <- !currentScope.sco_negofs - sizeOfType typ;
  !currentScope.sco_frame_place <- !currentScope.sco_frame_place + 1;
  let inf = {
    variable_type = typ;
    variable_offset = !currentScope.sco_negofs;
    variable_frame_place = !currentScope.sco_frame_place
  } in
  newEntry id (ENTRY_variable inf) (func_entry) err
...
let newFunction id err =
  ...
  with Not_found ->
    ...
    newEntry id (ENTRY_function inf) (ENTRY_function inf) false
  ...
  | ENTRY_function inf -> begin
      ...
      | PARDEF_DEFINE ->
          ...
          let inf_p = {
            ...
            parameter_offset = 0;
            parameter_mode = mode;
            parameter_frame_place = !currentScope.sco_frame_place
          } in
          let e = newEntry id (ENTRY_parameter inf_p) f.entry_info err in
...
let newTemporary typ =
  ...
  incr tempNumber;
  newEntry id (ENTRY_temporary inf) (ENTRY_temporary inf) false
